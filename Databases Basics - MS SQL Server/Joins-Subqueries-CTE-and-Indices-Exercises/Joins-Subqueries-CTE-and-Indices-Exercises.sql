...
SELECT TOP (5) e.EmployeeID, e.FirstName, p.Name
 ...
 WHERE p.StartDate > (SELECT CONVERT(DATE, '13.08.2002', 103)) AND p.EndDate IS NULL
 ORDER BY e.EmployeeID; 
...
HAVING c.CountryName IN ('United States', 
						 'Russia', 
						 'Bulgaria'); 
...
SELECT ranked.ContinentCode, 
  ...
  FROM (SELECT gbc.ContinentCode, 
		  ...
		  FROM (SELECT ContinentCode, 
			    ...
			    HAVING COUNT(CurrencyCode) > 1) AS gbc) AS ranked
...
SELECT TOP (5) peaks.CountryName, 
  ...
  FROM (SELECT c.CountryName, 
		  ...
		  FULL OUTER JOIN Peaks AS p
		    ON m.Id = p.MountainId) AS peaks
  FULL OUTER JOIN (SELECT c.CountryName, 
						  c.CountryCode, 
					 ...
					 FULL OUTER JOIN Rivers AS r
					   ON cr.RiverId = r.Id) AS rivers
    ON peaks.CountryCode = rivers.CountryCode
 WHERE peaks.DescendingElevationRank = 1 AND 
	   rivers.DescendingRiversLenghRank = 1 AND
	   (peaks.Elevation IS NOT NULL OR rivers.Length IS NOT NULL)
...
SELECT TOP (5) 
  ...
  FROM (SELECT c.CountryName, 
		  ...
		  LEFT JOIN Peaks AS p
		    ON m.Id = p.MountainId) AS jt
 WHERE jt.PeakRank = 1
 ORDER BY jt.CountryName, jt.PeakName

-- CASE solution

SELECT TOP (5) jt.CountryName AS Country, 
	   CASE
			WHEN jt.PeakName IS NULL THEN '(no highest peak)'
			ELSE jt.PeakName
	    END AS HighestPeakName, 
	   CASE 
			WHEN jt.Elevation IS NULL THEN 0
			ELSE jt.Elevation
	    END AS HighestPeakElevation, 
	   CASE 
			WHEN jt.MountainRange IS NULL THEN '(no mountain)'
